...
/// 因为以太网的 MTU 是 1500，除去 IP 头 20 字节、TCP 头 20 字节，还剩 1460；
/// 一般 TCP 包会包含一些 Option，IP 包也可能包含，所以预留 20 字节，再减去 4 字节长度，
/// 就是 1436，不用分片的最大消息长度，如果大于这个，很可能导致分片，所以压缩一下。
...
{
    // 把一个 Message encode 成一个 frame，写入 buf
    fn encode_frame(&self, buf: &mut BytesMut) -> Result<(), KvError> {
        ...
        if size > COMPRESSION_LIMIT {
            ...
            // ButesMut 支持逻辑上的 split，之后还能 unsplit
    ...
    // 把一个完整的 framde decode 成一个 Message
    ...
    S: AsyncRead + Unpin + Send 
{
    ...
    // 的这段内存没有初始化。我们就是为了 reserve 这段内存，然后从 stream
    // 里读取，读取完它就是初始化的，所以，我们这么用是安全的
