...
use certify::{CertType, load_ca, CA, generate_cert, generate_ca};
use anyhow::Result;
...
async fn gen_files(pem: &CertPem) -> Result<()> {
    ...
    fs::write(format!("fixtures/{}.cert", name), pem.cert.as_bytes())?;
    fs::write(format!("fixtures/{}.key", name), pem.key.as_bytes())?;
    Ok(())
}
